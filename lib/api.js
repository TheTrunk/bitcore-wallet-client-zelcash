...
var Mnemonic = require('bitcore-mnemonic-zelcash');
 ...
 *
 * @param {Object} opts
 * @param {String} opts.coin - default 'ZEL'
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  $.checkArgument(coin || _.includes(['ZEL'], coin));
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var coin = opts.coin || 'ZEL';
  if (!_.includes(['ZEL'], coin)) return cb(new Error('Invalid coin'));
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {
    if (!_.includes(['ZEL'], opts.coin)) return cb(new Error('Invalid coin'));
...
API.signTxProposalFromAirGapped = function(key, txp, unencryptedPkr, m, n, opts) {
  ...
  if (key.slice(0, 4) === 'xprv' || key.slice(0, 4) === 'tprv') {
    if (key.slice(0, 4) === 'xprv' && txp.network == 'testnet') throw new Error("testnet HD keys must start with tprv"); //TODO2
